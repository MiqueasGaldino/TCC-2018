...
void ThroughputTest:: calculo(double janela[], int pacotes[], int no, int contagem){
	...
	double vetor1[contagem/2];
	double vetor2[contagem/2];
	double median = mediana(janela,contagem);
	double vetor[contagem];
	int tamanho = 0;
    ...
    sort(vetor, vetor+contagem); 
	 ...
	 if (contagem % 2 == 0) {
	   ...
	   double quartil_1 = vetor1[(contagem/2)/2];
       double quartil_3 =  vetor2[(contagem/2)/2];
       
	   double iqr = quartil_3 - quartil_1;
	   
       double  limite_inferior = quartil_1 - 1.5*iqr;
       double limite_superior = quartil_3 + 1.5*iqr;
	...
	switch (index){
		case SEND_PACKET: {
			...
			if (addr.compare("0") != 0 and addr.compare("3") != 0 and addr.compare("4") != 0) {
				...
				if (addr.compare("1") == 0) {
					arquivo = "/home/miqueas/Dados_Encom/Multihop_in3.txt";
					}
				
				if (addr.compare("2") == 0){
					
					arquivo = "/home/miqueas/Dados_Encom/Multihop_out4.txt";
				...
				if (addr.compare("6") == 0){
					...
					arquivo = "/home/miqueas/Dados_Encom/Multihop_out2.txt";
